  ...
  <div class="app-container">
    <div class="block">
      ...
      <el-table :data="courseCommentList" border style="width: 100%">
        ...
        <el-table-column prop="content" label="评论" min-width="80" :show-overflow-tooltip="true"></el-table-column>
        ...
        <el-table-column align="center" label="审核">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              type="danger"
              @click="handleExamineNotPassComment(scope.row.id)"
            >拒绝</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    search: function () {
      ...
      this.httpPost("/video/courseComment/list", param).then(
        (res) => {
          this.courseCommentList = res.data.data;
          this.totalCount = res.data.total;
          this.$message({
    ...
    handleExamineNotPassComment(id) {
      this.$confirm("此操作将不通过审核评论, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.httpPost("/video/courseComment/examineNotPass", param).then(
            (res) => {
              this.$message({
                message: "审核不通过评论成功！",
                type: "success",
